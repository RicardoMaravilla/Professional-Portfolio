...
object ApiConnector {
    ...
    val api_url = envOrElse("API_URL","").toString().replace("\n", "")
    val api_token = envOrElse("API_TOKEN","").toString().replace("\n", "")
	...
	*/
    def send_json_doc(doc: String): Integer = {
        try{
            val resp = requests.post(
                api_url,
                data=doc,
                headers =Map("Authorization" -> s"Bearer ${api_token}", "Content-Type" -> "text/csv","Accept" -> "*/*")
            )
    ...
    */
    def send_json_doc_load_tries (doc: String, doc_type: String): Integer= {
        var retry_it = 0
        var resp = -1
