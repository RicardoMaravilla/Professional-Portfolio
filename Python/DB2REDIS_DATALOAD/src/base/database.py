...
def run_statement(conn, query):
    """
    Method for run a statement, receives a db2 connection and a query select
    and returns a result or False.
    Parameters
        - conn, connection to database
        - query, query to run
    Result
        On sucess, query_list(list): The result of the query
        On failure, false
    """
    query_list = []
    try:
        ...
        while result != False:
            query_list.append(result[0].replace(" ", ""))
            result = ibm_db.fetch_tuple(stmt)
    except Exception:
        logger.exception("Error trying to run query: %s", query)
        return False

    if not query_list:
        logger.error("The query %s dooesnt give results", query)
        return False

    logger.info("The query %s runs sucessfully", query)
    return query_list
